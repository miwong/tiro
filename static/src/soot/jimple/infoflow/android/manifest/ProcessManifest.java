 ...
 * Modifications:
 * - Add null check to handle cases where an attribute is not parsed properly
 *   and has no value.
 * - Check whether component is enabled when adding entry-points.
 *
...
public class ProcessManifest {
    ...
    private void checkAndAddComponent(Set<String> entryPoints, AXmlNode node) {
        ...
        if (attrExported != null && attrExported.getValue().equals(Boolean.FALSE)) {
            return;
        }

        // If using default exported value, check if component has declared an intent filter.
        if (attrExported == null) {
            if (node.getChildrenWithTag("intent-filter").isEmpty()) {
                return;
            }
        }
        ...
        if (attr != null)
            ...
            for (Entry<String, AXmlAttribute<?>> a : node.getAttributes().entrySet())
                if ((a.getValue().getName() == null || a.getValue().getName().isEmpty())
